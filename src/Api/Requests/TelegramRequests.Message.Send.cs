...
{
    public async Task<RequestResponse<Message>> SendText(
        long chatId,
        string text,
    ...
    {
        if (!await CanSendTemporary(chatId, lifeTime))
            return RequestResponse<Message>.UnSuccess();
        ...
        var request = await SendRequest<Message>(TelegramMethods.SEND_MESSAGE, parameters, ct);

        if (request.Ok)
            await ApplyLifetime(request.Result, chatId, lifeTime);

        return request;
    }

    public async Task<RequestResponse<Message>> SendMedia(
        long chatId,
        InputFile file,
        string? caption = null,
        IKeyboardMarkup? keyboard = null,
        ...
        CancellationToken ct = default)
    {
        ...
        var request = await SendRequest<Message>(method, parameters, ct);
    ...
    public async Task<RequestResponse<Message[]>> SendMediaGroup(
        long chatId,
        InputFile[] files,
        string? caption = null,
        IKeyboardMarkup? keyboard = null,
        ...
        CancellationToken ct = default)
    {
        ...
        var request = await SendRequest<Message[]>(TelegramMethods.SEND_MEDIA_GROUP, parameters, ct);

        if (request.Ok)
        {
            foreach (var res in request.Result!)
                await ApplyLifetime(res, chatId, lifeTime);
        }
